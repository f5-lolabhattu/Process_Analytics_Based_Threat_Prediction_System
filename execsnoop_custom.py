...
if args.print_uid:
    print("%-6s" % ("UID"), end="")
# print("%-18.9s %-16s %-7s %-7s %3s %3s %3s %s" % ("TSTAMP", "PCOMM", "PID", "PPID", "RET", "WR", "LAT", "ARGS"))
...
def print_event(cpu, data, size):
    ...
    if event.type == EventType.EVENT_ARG:
        ...
        if not skip:
            ...
            (task, pid, cpu, flags, wr_ts, msg) = b.trace_fields()

            wr_ts = wr_ts * 1000000000
            if event.ts >= wr_ts and current_pid != event.pid:
                ...
                # printb(b"%-18.9f %-16s %-7d %-7s %3d %2s %4.2f %s" % (time_s, event.comm, event.pid, ppid, event.retval, type_s, ms, argv_text))

                line = str(b"%-18.9f %-16s %-7d %-7s %3d %2s %4.2f %s" % (time_s, event.comm, event.pid, ppid, event.retval, type_s, ms, argv_text))

                command = line.split()[1]
                if command == 'curl' or len(current_url) == 0:
                    if len(current_url) > 0:
                        avg_curls[current_url] = url_dict[current_url] 

                    current_url = str(line.split()[-1])
                    url_dict[current_url] = 1

                if command == 'python':
                    url_dict[current_url] = url_dict[current_url] + 1
                
                
                if len(avg_curls.keys()) > 0:
                    for key, val in url_dict.items():
                        if key in avg_curls:
                            if avg_curls[key] > url_dict[key]:
                                print("Predict Threat @@@@@@@@@@@ ")
